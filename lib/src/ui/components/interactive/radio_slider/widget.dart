...
class _RadioSliderState extends State<RadioSlider> with TickerProviderStateMixin {
  ...
  void didUpdateWidget(RadioSlider oldWidget) {
    ...
    if (_index != widget.selectedIndex) {
      goToIndex(widget.selectedIndex);
    }
    if (widget.duration != _animation!.duration) {
      initController();
    }
  }
  ...
  void goToIndex(int index) {
    _index = index;
    _animation!.animateTo(indexVal(index), curve: Curves.easeOut);
  }
  ...
  void onDragUpdate(DragUpdateDetails details, double max) {
    _animation!.value += details.primaryDelta! / maxSpace(max);
    _sliding = true;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final double height = this.height(radioTheme);
