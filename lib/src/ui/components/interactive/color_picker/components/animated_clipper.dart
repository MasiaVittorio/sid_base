...
class _AnimatedClipperState extends State<AnimatedClipper> with TickerProviderStateMixin {
  ...
  void didUpdateWidget(AnimatedClipper oldWidget) {
    ...
    if (_controller.duration != widget.duration) {
      final valReset = _controller.value;
      _controller.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      child: FadeAndSizeTransition(
        ...
        child: Align(alignment: widget.childAlignment, child: widget.child),
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRect(
      child: Align(
        ...
        child: alsoFade ? Opacity(opacity: sizeFactor.value, child: child) : child,
      ),
    );
  }
}
