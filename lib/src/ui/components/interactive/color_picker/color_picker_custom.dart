...
class _CustomColorPickerState extends State<CustomColorPicker>
  ...
  Widget build(BuildContext context) => LayoutBuilder(
    builder: (context, externalConstraints) {
      final themeOfContext = Theme.of(context);
      final sliderThemeOfContext = SliderTheme.of(context);

      final bool big =
          CustomColorPicker.sliderHeight * 6 + 50 <=
          externalConstraints.maxHeight;
  ...
  Widget _displayer(bool big, ThemeData themeOfContext) =>
      _scrollable
          ? Expanded(
            child: LayoutBuilder(
              builder:
  ...
  Widget _rgbSlider(String rgb) {
    ...
    return AdvancedSlider(
      ...
      name: '$rgb: $number',
      annotation:
          '(${number.toRadixString(16).padLeft(2, '0').toUpperCase()} hex)',
      value: number.toDouble(),
      activeColor: baseColor,
      inactiveColor: baseColor.withValues(alpha: 0.24),
      max: 255.0,
      min: 0.0,
      onChanged: (double nd) {
        setState(() {
        ...
        child:
            _insertMode
                ...
                : InkWell(
                  ...
                  child: const Center(child: Icon(Icons.content_copy)),
                ),
      ),
    ],
  );

  Widget switcher(bool big) =>
      big
          ? Container()
          : InkWell(
            onTap: switchMode,
  ...
  Widget _realCenter(bool darkBkg) {
    String errorString =
        checkForHexString(_controller!.text) == false ? "Error" : "";
  ...
  Widget _hueSlider(Color activeColor, SliderThemeData sliderThemeOfContext) =>
      SliderTheme(
            ...
            gradient: LinearGradient(
              colors:
                  _interpolate(360, 0.0, 360)
                      .map<Color>(
                        (double x) =>
                            HSVColor.fromAHSV(1.0, x, 1.0, 1.0).toColor(),
                      )
                      .toList(),
            ),
          ),
  ...
  Widget _saturationSlider(
    Color activeColor,
    SliderThemeData sliderThemeOfContext,
  ) => SliderTheme(
    data: sliderThemeOfContext.copyWith(
      thumbColor: _color,
      thumbShape: BorderRoundSliderThumbShape(
      ...
      trackShape: ShadeRectangularSliderTrackShape(
        gradient: LinearGradient(
          colors:
              _interpolate(100, 0.0, 1.0)
                  .map<Color>(
                    (double x) =>
                        HSVColor.fromAHSV(1.0, _hue, x, _value).toColor(),
                  )
                  .toList(),
        ),
      ),
        ...
        gradient: LinearGradient(
          colors:
              _interpolate(100, 0.0, 1.0)
                  .map<Color>(
                    (double x) =>
                        HSVColor.fromAHSV(1.0, _hue, _sat, x).toColor(),
                  )
                  .toList(),
        ),
      ),
