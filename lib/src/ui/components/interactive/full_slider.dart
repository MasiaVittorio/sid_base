...
class FullSlider extends StatefulWidget {
  ...
  final Widget Function(double)?
      trailingBuilder; // if there is the builder, the single widget is ignored
...
class _FullSliderState extends State<FullSlider>
  ...
  double widgetToAnimation(double widgetValue) => widgetValue
      .mapToRange(0.0, 1.0, fromMin: widget.min, fromMax: widget.max);
  ...
  bool get withLeading =>
      widget.leading != null || widget.leadingBuilder != null;
  bool get withTrailing =>
      widget.trailing != null || widget.trailingBuilder != null;

  bool get disabled => !widget.enabled;

  double get dividedControllerValue =>
      (controller!.value * widget.divisions!).round() / widget.divisions!;
  ...
  Widget build(BuildContext context) {
    ...
    final Color background = Color.alphaBlend(
      ...
      themeData
          .canvasColor, // this widget is intended to be put over a material background
    );
    final Color accentColor = Color.alphaBlend(
      widget.accentColor ??
          themeData.colorScheme.secondary.withOpacity(disabled ? 0.17 : 0.2),
      background,
    );
              ...
              children: <Widget>[
                ...
                if (withTrailing)
                  Container(
                    height: widget.crossAxisSize,
                    width:
                        widget.unboundedTrailing ? null : widget.crossAxisSize,
                    alignment: Alignment.center,
          ...
          return Container(
            ...
            child: ConstrainedBox(
              constraints: BoxConstraints(
                maxWidth:
                    horizontal ? constraints.maxWidth : widget.crossAxisSize,
                maxHeight:
                    vertical ? constraints.maxHeight : widget.crossAxisSize,
              ),
              child: GestureDetector(
                onHorizontalDragUpdate: disabled || vertical
                    ? null
                    : (details) =>
                        dragUpdate(details, delta - widget.crossAxisSize),
                onVerticalDragUpdate: disabled || horizontal
                    ? null
                    : (details) =>
                        dragUpdate(details, delta - widget.crossAxisSize),
                onHorizontalDragEnd:
                    disabled || vertical ? null : (_) => dragEnd(),
                onVerticalDragEnd:
                    disabled || horizontal ? null : (_) => dragEnd(),
                onHorizontalDragCancel:
                    disabled || vertical || widget.divisions != null
                        ? null
                        : dragEnd,
                onVerticalDragCancel:
                    disabled || horizontal || widget.divisions != null
                        ? null
                        : dragEnd,
                onTapUp: disabled || widget.divisions == null
                    ? null
                    : (d) => tap(d, constraints),
                child: Stack(
                  children: <Widget>[
                    Positioned.fill(
                      ...
                      decoration: BoxDecoration(
                        color: widget.overrideBackgroundColor ?? background,
                        borderRadius: BorderRadius.circular(widget.radius),
                      ),
                      child: bottomContent,
                    )),
                        ...
                        child: ClipRRect(
                          ...
                          child: AnimatedBuilder(
                            ...
                            builder: (_, __) {
                              ...
                              return Align(
                                alignment: aligment,
                                widthFactor: horizontal
                                    ? val.mapToRange(
                                        widget.crossAxisSize / delta, 1.0)
                                    : null,
                                heightFactor: vertical
                                    ? val.mapToRange(
                                        widget.crossAxisSize / delta, 1.0)
                                    : null,
                                child: topContent,
