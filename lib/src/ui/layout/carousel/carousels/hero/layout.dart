...
class HeroLayouter extends M3CarouselLayouter<HeroItemState> {
  ...
  @override
  (Positioner, HeroItemState)? position(int i, double x) {
    return _steps(
  ...
  (Positioner, HeroItemState)? _future() {
    return _position(start: D, end: D + t, state: const HeroThinItem(0));
  }
  ...
  (Positioner, HeroItemState) _futureToSmall(double value) {
    ...
    return _position(
      ...
      state: HeroThinItem(
        value <= tF ? 0 : value.rangeMap(from: (thinFrac, 1)),
      ),
    );
  }
  ...
  (Positioner, HeroItemState) _centerToPast(double value) {
    return _position(
      start: value.rangeMap(to: (A, -pastWidth)),
      end: value.rangeMap(to: (D - Z - s - b, 0)),
  ...
  (Positioner, HeroItemState)? _past() {
    return _position(start: -pastWidth, end: 0, state: const HeroPastItem());
  }

  (Positioner, HeroItemState)? _steps(
    ({num min, (Positioner, HeroItemState)? Function() positioner}) onFuture,
    List<
      ({
        num min,
        num max,
